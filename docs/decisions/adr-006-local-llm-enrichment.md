...
7. **Licensing** (see `docs/reference/licensing.md`): node-llama-cpp is MIT; its vendored llama.cpp copy (pinned 3.20.0) is MIT — AutoDoc is a developer/pipeline tool, not an embedded library, so even a future GPL llama.cpp bump would only matter on redistribution. GGUF weights are fetched per-host (Qwen: Apache-2.0; Gemma: commercial use allowed, weights not redistributable in artifacts). Re-verify `node_modules/node-llama-cpp/llama/llama.cpp/LICENSE` after any node-llama-cpp version bump.
8. Execution records: ai-memory `notes/gpu-acceleration-live.md`, `notes/cuda-gcc16-resolution.md`.